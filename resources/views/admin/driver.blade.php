  ...
  <div class="card-body fs-6 py-15 px-10 py-lg-15 px-lg-15 text-gray-700">
    ...
    <div>
      ...
      <div class="col-12 d-flex">
        <h1 class="me-auto anchor fw-bolder mb-5" id="striped-rounded-bordered">Data driver</h1>
        ...
        <table id="myTable" class="table table-striped table-hover table-rounded border gs-7">
          <thead>
            <tr class="fw-bold fs-6 text-gray-800 border-bottom border-gray-200">
              ...
              <th>Nama</th>
              <th style="min-width: 120px">Kantor cabang</th>
              <th style="min-width: 120px">Nomor SIM</th>
          ...
          <tbody>
            ...
            @php
              $updated = date_create($d->updated_at);
            @endphp
            <tr>
              <td class="">{{$loop->iteration}}</td>
              <td style="min-width: 320px;">{{ $d->name }}  </td>
              <td>{{ $d->office->name }}</td>
              <td>
                <span class="badge badge-primary">{{ $d->license_number }}</span>
              </td>
              <td>
                <span class="badge badge-success">{{ $d->phone }}</span>
              </td>
              <td>{{date_format($updated,"d F Y")}}</td>
              <td>
                <a href="#" class="btn btn-icon btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#edit" onclick="edit({{ $d->id }})"><i class="bi bi-pencil-fill"></i></a>
                <a href="#" class="btn btn-icon btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#hapus" onclick="hapus({{ $d->id }})"><i class="fa fa-times"></i></a>
              </td>
            </tr>
            @endforeach
          </tbody>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title">Tambah driver</h3>
